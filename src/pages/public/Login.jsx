...
import { Eye, EyeOff, Building2, GraduationCap, ArrowRight } from 'lucide-react'
import { loginCompany, loginSchool } from '../../api'
...
export default function Login() {
  ...
  // Admin tab removed — admin has a separate login page
  const tabs = [
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      if (role === 'company') {
        ...
        const { access_token, refresh_token, company } = res.data
        ...
        const { access_token, refresh_token, school } = res.data
        localStorage.setItem('token', access_token)
        localStorage.setItem('refresh_token', refresh_token)
    ...
    <div className="min-h-screen flex" style={{ background: 'linear-gradient(135deg, #F9F6F0 0%, #e8f5e9 100%)' }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 gradient-hero items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative text-center max-w-sm">
          ...
          <p className="text-white/60 text-base leading-relaxed">
            Continue connecting Ghana's schools and companies for meaningful internship experiences.
          </p>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          ...
          {/* Role tabs — School and Company only */}
            ...
            {tabs.map(({ key, label, icon: Icon }) => (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                  role === key ? 'bg-white text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'
                }`}>
                <Icon size={15} />
                {label}
              </button>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className={`input-field pr-12 ${errors.password ? 'border-red-300 focus:ring-red-200' : ''}`}
                />
                <button
                  type="button"
          ...
          <p className="text-center text-sm text-text-secondary mt-6">
            ...
            <Link
              to={role === 'school' ? '/register-school' : '/register-company'}
